...
namespace SpacerUnion.Common
{
    public partial class Localizator
    {
        public static void Init()
        {
            ...
            AddNewWord("VOB_ALIGN_TO_GROUND", new List<string> { "Выровнять воб по нормали поверхности", "Align vob to ground normal", "Vob an der Bodennormalen ausrichten", "Wyrównaj voba do normalnej podłoża", "Zarovnat vob na normálu povrchu" });
            AddNewWord("VOB_ALIGNED_TO_GROUND", new List<string> { "Воб выровнен по нормали поверхности", "Vob aligned to ground normal", "Vob an der Bodennormalen ausgerichtet", "Vob wyrównany do normalnej podłoża", "Vob zarovnán na normálu povrchu" });
            AddNewWord("VOB_ALIGN_TO_GROUND_FAILED", new List<string> { "Не удалось выровнять воб по нормали поверхности", "Failed to align vob to ground normal", "Fehler beim Ausrichten des Vobs an der Bodennormalen", "Nie udało się wyrównać voba do normalnej podłoża", "Selhalo zarovnání vobu na normálu povrchu" });
            ...
            AddNewWord("MENU_TOP_CREATE_VOB_VISUALS_LIST", new List<string> { "Сформировать файл списка визуалов локации (HTML)", "Create (HTML) file of visuals of the location", "Erstelle eine Liste der Dateien, die in diesem Bereich verwendet werden (HTML)", "Stwórz lisę plików, któresą używane w lokacji (HTML)", "Vytvoření seznamu souborů, které se používají v této lokaci (HTML)" });
            ...
            AddNewWord("TT_MISC_DATE_PREFIX", new List<string> { "Добавляет префикс с текущей датой к имени файла при сохранении (формат: ГГГГ-ММ-ДД_)", "Prefixes saved ZEN filenames with the current date (format: yyyy-mm-dd_)", "Stellt das aktuelle Datum vor den Dateinamen beim Speichern (Format: jjjj-mm-tt_)", "", "" });
            AddNewWord("TT_MISC_EXIT_ASK", new List<string> { "Показывает диалог подтверждения перед закрытием редактора, если загружен ZEN", "Shows a confirmation dialog before closing the editor while a world is loaded", "Zeigt einen Bestätigungsdialog vor dem Schließen, wenn eine Welt geladen ist", "", "" });
            AddNewWord("TT_MISC_FULL_PATH", new List<string> { "Показывает полный путь к загруженному ZEN в заголовке главного окна", "Shows the full path to the loaded ZEN file in the main window title bar", "Zeigt den vollständigen Pfad zur geladenen ZEN-Datei in der Titelleiste", "", "" });
            AddNewWord("TT_MISC_AUTO_COMPILE", new List<string> { "Автоматически компилирует геометрию и освещение после слияния меша с вобами", "Automatically compiles world geometry and lighting after the mesh-with-vobs merge step", "Kompiliert Welt-Geometrie und Beleuchtung automatisch nach dem Mesh-Vob-Merge", "", "" });
            AddNewWord("TT_MISC_AUTO_COMPILE_UNCOMPILED", new List<string> { "Автоматически компилирует некомпилированные ZEN при загрузке — экономит ручной шаг", "Automatically compiles uncompiled ZEN files on load — saves a manual compile step", "Kompiliert nicht kompilierte ZEN-Dateien beim Laden automatisch — spart einen manuellen Schritt", "", "" });
            AddNewWord("TT_MISC_AUTO_REMOVE_VISUALS", new List<string> { "Очищает поле visual у всех zCVobLevelCompo перед сохранением — уменьшает размер ZEN", "Clears the Visual field on all zCVobLevelCompo before saving — keeps ZEN files smaller", "Entfernt das Visual-Feld bei allen zCVobLevelCompo vor dem Speichern — kleinere ZEN-Dateien", "", "" });
            AddNewWord("TT_MISC_REMOVE_LEVELCOMPOS", new List<string> { "Удаляет все zCVobLevelCompo при загрузке ZEN — полезно для очистки старых миров", "Strips all zCVobLevelCompo containers on ZEN load — useful for cleaning up legacy worlds", "Entfernt alle zCVobLevelCompo-Container beim Laden — nützlich für alte Welten", "", "" });
            AddNewWord("TT_MISC_NEAREST_VOB_CAM", new List<string> { "После загрузки помещает камеру на VOB с именем VOB_SPACER_CAMERA_START или ближайший zCVobStartpoint", "After load, snaps the camera to a VOB named VOB_SPACER_CAMERA_START or to the nearest zCVobStartpoint", "Setzt nach dem Laden die Kamera auf einen VOB namens VOB_SPACER_CAMERA_START oder den nächsten zCVobStartpoint", "", "" });
            AddNewWord("TT_MISC_SHOW_POLYS_SORT", new List<string> { "Спрашивает о сортировке полигонов при сохранении больших локаций (>200 тыс. полигонов) — сортировка занимает время", "Asks before sorting polygons when saving large locations (>200k polys) — sorting is slow", "Fragt vor der Polygon-Sortierung beim Speichern großer Welten (>200k Polygone) — Sortierung dauert", "", "" });
            AddNewWord("TT_MISC_ONLY_LATIN", new List<string> { "Ограничивает ввод в поля свойств латинскими символами", "Restricts property-field input to Latin characters", "Beschränkt Eingaben in Eigenschafts-Feldern auf lateinische Zeichen", "", "" });
            AddNewWord("TT_MISC_UPPERCASE", new List<string> { "Переводит некоторые поля воба в верхний регистр при сохранении — совместимость движка со старыми ассетами", "Uppercases certain VOB fields when saving — engine compatibility for legacy assets", "Wandelt bestimmte VOB-Felder beim Speichern in Großbuchstaben um — Engine-Kompatibilität", "", "" });
            AddNewWord("TT_MISC_DISABLE_UPPER", new List<string> { "Запрещает преобразование значений свойств воба в верхний регистр при отображении", "Prevents VOB property values from being displayed in uppercase", "Verhindert die Umwandlung von VOB-Eigenschaftswerten in Großbuchstaben bei der Anzeige", "", "" });
            AddNewWord("TT_MISC_FAST_3DS", new List<string> { "Ускоряет загрузку 3DS-локаций примерно на 30%. Безопасно только для рабочих версий ZEN, не для финальной сборки", "Speeds up 3DS location loading by ~30%. Safe only for working ZEN versions, not final builds", "Beschleunigt das Laden von 3DS-Welten um ~30%. Nur für Arbeitsversionen sicher, nicht für Final-Builds", "", "" });
            AddNewWord("TT_MISC_DX11_AMB", new List<string> { "Включает источник внешнего освещения для DirectX 11. NumPad +/- меняет радиус", "Enables an ambient light source for the DirectX 11 renderer. NumPad +/- adjusts the radius", "Aktiviert eine Umgebungslichtquelle für den DirectX-11-Renderer. NumPad +/- ändert den Radius", "", "" });
            AddNewWord("TT_MISC_SKIP_POLYS_CUT", new List<string> { "Сохраняет «лишние» полигоны вместо удаления. Выключите, чтобы избежать дыр в меше; включите для совместимости со старыми локациями", "Keeps redundant polygons instead of removing them. Off avoids mesh holes; on preserves legacy compatibility", "Behält redundante Polygone statt sie zu entfernen. Aus vermeidet Mesh-Löcher, an für Legacy-Kompatibilität", "", "" });
            AddNewWord("TT_MISC_NO_WORK_CHECK", new List<string> { "Пропускает проверку пути при загрузке ZEN — позволяет загружать файлы вне папки _WORK", "Skips path validation on ZEN load — allows loading files from outside the _WORK folder", "Überspringt die Pfad-Prüfung beim Laden — erlaubt ZEN-Dateien außerhalb von _WORK", "", "" });
            ...
            AddNewWord("WIN_INFO_SHOW_ACTION_TRIGGER_CHANGED", new List<string> { "Trigger был изменен", "Trigger was changed", "Trigger wurde geändert", "Trigger został zmieniony", "" });
            AddNewWord("WIN_INFO_SHOW_ACTION_TRIGGER_RUN", new List<string> { "Trigger был запущен", "Trigger was started", "Trigger wurde gestartet", "Trigger został uruchomiony", "" });
            AddNewWord("WIN_INFO_SHOW_ACTION_SET_GLOBAL_PARENT", new List<string> { "Воб был назначен Глобальным Родителем", "Vob was set as the Global Parent", "Vob wurde als globales Parent gesetzt", "Vob został ustawiony jako Global Parent", "" });
            AddNewWord("WIN_INFO_SHOW_ACTION_NULL_GLOBAL_PARENT", new List<string> { "Глобальный Родитель был очищен", "Global Parent has been cleared", "Global Parent wurde entfernt", "Global Parent został wyczyszczony", "" });
