...
namespace SpacerUnion.Common
{
    public partial class Localizator
    {
        public static void Init()
        {
            ...
            words.Add("WIN_VOBCATALOG_TITLE", new List<string> { "Каталог вобов", "Vobs catalog", "VOBs Katalog", "Katalog vobów", "Katalog vobů" });
            words.Add("WIN_VOBCATALOG_GROUP_NAME", new List<string> { "Имя группы: ", "Group name: ", "Gruppen Name: ", "Nazwa grupy: ", "Jméno skupiny: " });
            words.Add("WIN_VOBCATALOG_VISUAL_NAME", new List<string> { "Визуал: ", "Visual: ", "Visuelles: ", "Visual: ", "Vizuál: " });
            words.Add("WIN_VOBCATALOG_MATFILTER_TURNOFF", new List<string> { "Сначала закройте окно Фильтра материалов!", "Turn off Materials filter window first!", "Schalte zuerst das Materialfilterfenster aus!", "Wpierw zamknij okno filtr materiałów!", "Nejprve vypněte okno filtru materiálů!" });
            words.Add("WIN_VOBCATALOG_POLYSELECT_TURNOFF", new List<string> { "Сначала выключите режим выделения полигонов!", "Turn off polygon select mod first!", "Schalte zuerst den Polygonauswahlmodus aus!", "Wpierw wyłącz tryb wyboru poligonów", "Nejprve vypněte režim výběru polygonu!" });
            words.Add("WIN_VOBCATALOG_ASKSURE_REMOVE_GROUP", new List<string> { "Вы уверены? Все вобы в группе будут удалены!", "Are you sure? All vobs in the group will be deleted!", "Bist du sicher? Alle VOBs in der Gruppe werden gelöscht!", "Jesteś pewien? Wszystkie voby w grubie zostaną skasowane!", "Jste si jistý? Všechny voby ve skupině budou smazány!" });
            words.Add("WIN_VOBCATALOG_IS_STATIC_VOB", new List<string> { "Статичный воб (isStatic): ", "Static vob (isStatic): ", "Statische VOB (isStatic): ", "Vob statyczny (isStatic): ", "Statický vob (isStatic): " });
            words.Add("WIN_VOBCATALOG_BAD_SYMBOL", new List<string> { "Нельзя использовать символ ;", "Symbol ; is not allowed here", "Symbol ; ist nicht erlaubt", "Znak ; nie jest dozwolony w tym miejscu", "Symbol ; zde není povolen" });
            words.Add("WIN_VOBCATALOG_HIDE_PREVIEW", new List<string> { "Скрывать 3D превью-модель при скрытии окна", "Hide 3D preview when window is hidden", "3D-Vorschau ausblenden, wenn das Fenster ausgeblendet ist", "Schowaj podgląd 3d gdy okno jest schowane", "Skrytí 3D náhledu při skrytém okně" });

            words.Add("WIN_VOBCATALOG_GROUPS", new List<string> { "Группы", "Groups", "Gruppen", "Grupy", "Skupiny" });
            words.Add("WIN_VOBCATALOG_ITEMS", new List<string> { "Элементы", "Entries", "Einträge", "Wpisy", "Záznamy" });
            words.Add("WIN_VOBCATALOG_ACTIONS", new List<string> { "Действия", "Actions", "Aktionen", "Akcje", "Akce" });

            words.Add("WIN_VOBCATALOG_ALL_COUNT", new List<string> { "Всего объектов: ", "Objects amount: ", "Objektanzahl: ", "Ilość obiektów: ", "Počet objektů: " });
            words.Add("WIN_VOBCATALOG_SEARCH", new List<string> { "Поиск: ", "Search: ", "Suche: ", "Szukaj: ", "Hledat: " });

            words.Add("WIN_VOBCATALOG_BUTTON_SAVE", new List<string> { "Сохранить копию базы каталога", "Save a copy of Vob Catalog", "Eine Kopie des VOB-Katalogs speichern", "Zapisz kopie katalogu vobów", "Uložit kopii Katalogu Vobů" });
            ...
            words.Add("WIN_VOBCATALOG_BTN_ADD_GROUP_NEW", new List<string> { "Добавить новую группу", "Add new group", "Neue Gruppe hinzufügen", "Dodaj nową grupę", "Přidat novou skupinu" });
            words.Add("WIN_VOBCATALOG_BTN_RENAME_GROUP", new List<string> { "Переименовать группу", "Rename group", "Gruppe umbenennen", "Zmień nazwę grupy", "Přejmenovat skupinu" });
            words.Add("WIN_VOBCATALOG_BTN_REMOVE_GROUP", new List<string> { "Удалить группу", "Delete group", "Gruppe löschen", "Usuń grupę", "Odstranit skupinu" });


            words.Add("WIN_VOBCATALOG_BTN_ELEM_ADD", new List<string> { "Добавить новый", "Add new", "Neu hinzufügen", "Dodaj nowe", "Přidat nový" });
            words.Add("WIN_VOBCATALOG_BTN_ELEM_EDIT", new List<string> { "Изменить", "Edit", "Bearbeiten", "Edytuj", "Editovat" });
            words.Add("WIN_VOBCATALOG_BTN_ELEM_REMOVE", new List<string> { "Удалить", "Remove", "Entfernen", "Usuń", "Odstranit" });

            words.Add("WIN_VOBCATALOG_IN_CURRENT_GROUP", new List<string> { "В выделенной группе: ", "In selected group: ", "In ausgewählter Gruppe: ", "W zaznaczonej grupie: ", "Ve vybrané skupině: " });
            ...
            words.Add("WIN_VOBCATALOG_ASKSURE_CMD_MODELS", new List<string> { "Данная функция выведет в консоль визуалы вобов, для которых нет файлов моделей в VDF/_WORK. Продолжить?", "This function will display vobs' visuals for which there are no model files in VDF/_WORK to the console. Continue?", "Mit dieser Funktion werden visuelle VOBs, für die es keine Modelldateien in VDF/_WORK gibt, auf der Konsole angezeigt. Weiter?", "Ta funkcja wyświetli na konsoli wizualne voby, dla których nie ma plików modelu w VDF/_WORK. Kontynuować?", "Tato funkce zobrazí vizuální voby pro které neexistují modelové soubory ve složce VDF/_WORK, do konzole. Pokračovat?" });
            words.Add("WIN_VOBCATALOG_ADV", new List<string> { "Продвинутые", "Advanced", "Erweitert", "Zaawansowane", "Pokročilé" });

            words.Add("WIN_VOBCATALOG_ADV_DONT_SEARCH", new List<string> { "Не показывать Вобы Каталога в глобальном поиске", "Don't show Catalog's vobs in global search", "Keine Katalog VOBs in allgemeiner Suche anzeigen", "Nie pokazuj vobsów Katalogu w wyszukiwaniu globalnym", "Nezobrazovat Katalog vobů v globálním vyhledávání" });
            ...
            words.Add("WIN_VOBCATALOG_COPY_VISUAL", new List<string> { "Скопировать визуал в другую группу...", "Copy visual to another group...",  "Visuelles in eine andere Gruppe kopieren...", "Skopiuj visual do innej grupy...", "Zkopírovat vizuál do jiné skupiny..." });
            words.Add("WIN_VOBCATALOG_MOVE_VISUAL", new List<string> { "Переместить визуал в другую группу...", "Move visual to another group...",  "Visuelles in eine andere Gruppe verschieben...", "Przenieś visual do innej grupy...", "Přesunout vizuál do jiné skupiny..." });
            ...
            words.Add("ERROR_REPORT_PROBLEM_TYPE_FOCUS_NAME", new List<string> { "Такой 'focusName' не существует в скриптах", "Such 'focusName' does not exist in scripts", "Ein solcher 'focusName' existiert nicht in den Skripten", "Taki 'focusName' nie istnieje w skryptach", "Takový 'focusName' ve skriptech neexistuje" });
            words.Add("ERROR_REPORT_PROBLEM_TYPE_KEY_INSTANCE", new List<string> { "Такой 'keyInstance' не существует в скриптах", "Such 'keyInstance' does not exist in scripts", "Eine solche 'keyInstance' existiert nicht in den Skripten", "Taki 'keyInstance' nie istnieje w skryptach", "Taková 'keyInstance' ve skriptech neexistuje" });
            words.Add("ERROR_REPORT_PROBLEM_TYPE_CONTAINTER_ITEM", new List<string> { "Такой oCItem из 'contains' не существует в скриптах", "Such oCItem in 'contains' does not exist", "Ein solches oCItem in 'contains' existiert nicht", "Taki oCItem w 'contains' nie istnieje", "Takový oCItem v 'contains' neexistuje" });
            words.Add("ERROR_REPORT_PROBLEM_TYPE_SPACES_CONTAINER", new List<string> { "Поле 'contains' содержит пробелы", "Field 'contains' has spaces", "Feld 'contains' hat Leerzeichen", "Pole 'constains' zawiera spacje", "Pole 'contains' obsahuje mezery" });
            words.Add("ERROR_REPORT_PROBLEM_TYPE_SOUNDNAME", new List<string> { "Поле 'sndName' пустое", "Field 'sndName' is empty", "Feld 'sndName' ist leer", "Pole 'sndName' jest puste", "Pole 'sndName' je prázdné" });
            ...
            words.Add("WIN_SPAWN_ASKSURE_REMOVE_GROUP", new List<string> { "Вы уверены? Все функции в пресете будут удалены!", "Are you sure? All functions in the preset will be deleted!", "Bist du sicher? Alle Funkionen in der Voreinstellung werden gelöscht!", "Czy na pewno? Wszystkie funkcje w presecie zostaną usunięte!", "Jste si jistý? Všechny funkce v předvolbě budou smazány!" });
            words.Add("WIN_SPAWN_FUNC_NAME", new List<string> { "Имя функции: ", "Function name: ", "Funktion Name: ", " Nazwa funkcji: ", "Název funkce: " });
            words.Add("WIN_SPAWN_NO_GROUP_SELECTED", new List<string> { "Сначала выберите любой пресет!", "Select any preset first!", "Wähle zuerst eine Voreinstellung aus!", " Wybierz najpierw pierwszy preset!", "Nejprve vyberte libovolnou předvolbu!" });
            words.Add("WIN_SPAWN_NO_HELP_VOBS", new List<string> { "Сначала включите режим показа help-вобов!", "Activate help vobs show mode!", "Aktiviere den Hilfe VOBs Anzeige Modus!", "Aktywuj tryb wyświetlania vobów pomocniczych!", "Aktivujte režim zobrazení nápovědy vobů!" });
            words.Add("WIN_SPAWN_RADIUS_RANGE", new List<string> { "Достустимый диапазон от 2000 до 25000", "Allowed range is from 2000 to 25000", "Der zulässige Bereich liegt zwischen 2000 und 25000", "Dozwolony zakres wynosi od 2000 do 25000", "Povolený rozsah je 2000 až 25000" });

            words.Add("WIN_OBJ_TAB8", new List<string> { "Спавн", "Spawn", "Spawn", "Spawn", "Spawn" });
            words.Add("WIN_SPAWN_PRESETS", new List<string> { "Пресеты", "Presets", "Voreinstellungen", "Presets", "Předvolby" });
            words.Add("WIN_SPAWN_SCRIPT_FUNCTIONS", new List<string> { "Скриптовые функции:", "Script functions:", "Skript Funktionen:", "Funkcje skryptu:", "Funkce skriptu:" });
            words.Add("WIN_SPAWN_LABEL_RADIUS", new List<string> { "Радиус показа: ", "Show radius: ", "Zeige Radius: ", "Pokaż promień: ", "Zobrazit poloměr: " });
            words.Add("WIN_SPAWN_BUTTON_RADIUS", new List<string> { "Установить", "Set", "Festlegen", "Ustaw", "Nastavit" });
            words.Add("WIN_SPAWN_BUTTON_CLEAR", new List<string> { "Очистить точки", "Clear points", "Lösche Punkte", "Wyczyść punkty", "Vymazat body" });
            words.Add("WIN_SPAWN_BUTTON_SHOW", new List<string> { "Показать точки спавна (WP/FP)", "Show spawn points (WP/FP)", "Spawnpunkte anzeigen (WB/FP)", "Pokaż punkty spawnu (WP/FP)", "Zobrazit spawn body (WP/FP)" });
            words.Add("WIN_SPAWN_GROUPBOX_FUNCTIONS", new List<string> { "Функции", "Functions", "Funktionen", "Funkcje", "Funkce" });

            words.Add("WIN_SPAWN_SAVE_FILE", new List<string> { "Сохранить файл пресетов", "Save presets file", "Speichere Voreinstellungsdatei", "Zapisz plik presets", "Uložení souboru předvoleb" });
            words.Add("WIN_SPAWN_ADD_NEW_FUNCTION", new List<string> { "Добавить новую", "Add new", "Neu hinzufügen", "Dodaj nowy", "Přidat nové" });
            ...
            words.Add("WARNING_3DS_FAST_MODE", new List<string> { "3DS файл был загружен в 'Быстром режиме'", "3DS file was loaded with 'Fast mod'", "3DS-Datei wurde im 'Schnellmodus' geladen", "Plik 3DS został załadowany w szybkim trybie", "3DS soubor byl načten v „Rychlém Režimu“" });
            ...
            words.Add("UV_WIN_TITLE", new List<string> { "Окно поиска проблем в UV развертке", "Finding UV wrapping problems", "Suchfenster von UV-Mapping-Problemen", "Okno wyszukiwania problemów z mapowaniem UV", "Vyhledávání problémů s UV obalem" });
            words.Add("UV_WIN_DESCR_1", new List<string> { "Данный инструмент может найти проблемы в UV развертке в меше локации", "This tool can find problems in UV mapping in a location mesh", "Dieses Werkzeug kann Probleme im UV-Mapping eines Standort-Meshes finden", "To narzędzie może znaleźć problemy w mapowaniu UV w siatce mesha lokacji", "Tento nástroj dokáže najít problémy v mapování UV v síti umístění" });
            words.Add("UV_WIN_DESCR_2", new List<string> { "Внимание! Инструмент показывает ПРЕДПОЛАГАЕМЫЕ проблемы.", "Attention! The tool shows POTENTIAL problems.", "Achtung! Das Werkzeug zeigt MÖGLICHE Probleme an.", "Uwaga! To narzędzie pokazuje POTENCJALNE problemy.", "Pozor! Nástroj zobrazuje POTENCIÁLNÍ problémy." });
            words.Add("UV_WIN_DESCR_3", new List<string> { "UV развертку найденного полигона вы оцениваете визуально самостоятельно! ", "You must visually check the UV mapping of the found polygon!", "Du musst das UV-Mapping des gefundenen Polygons selbst visuell überprüfen!", "Należy wizualnie sprawdzić mapowanie UV znalezionego poligonu!", "Musíte vizuálně zkontrolovat UV mapování nalezeného polygonu!" });
            words.Add("UV_WIN_BUTTON_FIND_UV", new List<string> { "Найти проблемы UV", "Find UV problems", "UV-Probleme finden", "Znajdź problemy z UV", "Vyhledávání problémů s UV zářením" });
            words.Add("UV_WIN_MIN_AREA", new List<string> { "Минимальная площадь UV. Плохие развертки имеют значение ≤ 0.001", "Minimal UV area. Bad UV's have value ≤ 0.001", "Minimale UV-Fläche. Schlechte UVs haben einen Wert ≤ 0,001", "Minimalny zakres UV. Złe UV mają wartość ≤ 0.001", "" });
            words.Add("UV_WIN_MAX_AREA", new List<string> { "Максимальная площадь UV. Плохие развертки имеют значение ≥ 5 (не всегда)", "Max UV area. Bad UV's can have value ≥ 5 (not always)", "Maximale UV-Fläche. Schlechte UVs können einen Wert ≥ 5 haben (nicht immer)", "Maksymalny zakres UV. Złe UV mają wartość ≥ 5 (nie zawsze)", "" });
            words.Add("UV_WIN_MAX_ANGLE_DIST", new List<string> { "Угол искажения (от 1 до 179). Плохие развертки имеют значение ≥ 30 (не всегда)", "Distortion angle (from 0 to 179). Bad UV's can have value ≥ 30 (not always)", "Verzerrungswinkel (von 1 bis 179). Schlechte UVs können einen Wert ≥ 30 haben (nicht immer)", "Kąt zniekształcenia (od 0 do 179). Złe UV mogą mieć wartość ≥ 30 (nie zawsze)", "" });
            words.Add("UV_WIN_IGNORE_NOCOLL", new List<string> { "Игнорировать материалы/полигоны без коллизии", "Ignore materials/polygons without collision", "Materialien/Polygone ohne Kollision ignorieren", "Ignoruj materiały/poligony bez kolizji", "" });
            words.Add("UV_WIN_POLY_SELECT_MUST_BE_ON", new List<string> { "Сначала ВКЛЮЧИТЕ режим выделения полигонов!", "Turn ON polygon select mod first!", "Schalte zuerst den Polygon-Auswahlmodus EIN!", "Najpierw włącz tryb zaznaczania poligonów!", "" });
            words.Add("UV_WIN_SLIDER_DIST", new List<string> { "Радиус отрисовки 'плохих' полигонов: ", "Draw distance of 'bad' polygons: ", "Darstellungsradius der „schlechten“ Polygone: ", "Zasięg rysowania 'złych' poligonów: ", "" });
            ...
            words.Add("WIN_VOBCATALOG_COMPARE_BUTTON", new List<string> { "Сравнить динам. коллизию", "Compare dyn. collision", "Dynamische Kollision vergleichen", "Porównaj dynamiczną kolizje", "" });
            ...
            words.Add("ERROR_REPORT_PROBLEM_TYPE_TGA_DYNCOLL", new List<string> { "У воба c TGA визуалом динамическая коллизия. Вы уверены?", "Vob with TGA visual has dynamic collision. Are you sure?", "Der Vob mit einem TGA-Visual hat eine dynamische Kollision. Bist du sicher?", "Vob z visualem TGA ma dynamiczną kolizje. Jesteś pewien?", "" });
            words.Add("ERROR_REPORT_PROBLEM_TYPE_PFX_CANT_HAVE_DYNCOLL", new List<string> { "У PFX не может быть динамической коллизии!", "PFX can't have dynamic collision!", "Ein PFX kann keine dynamische Kollision haben!", "PFX nie może mieć dynamicznej kolizji!", "" });
            words.Add("ERROR_REPORT_PROBLEM_TYPE_BAD_ZFARVOB", new List<string> { "Подозрительное значение поля 'vobFarClipZScale'. Оно должно быть от 0 до 3", "Suspicious value of field 'vobFarClipZScale'. It should be from 0 to 3", "Verdächtiger Wert im Feld 'vobFarClipZScale'. Er muss zwischen 0 und 3 liegen.", "Podejrzana wartość pola 'vobFarClipZScale'. Oczekiwana wartość w zakresie od 0 do 3", "" });

            words.Add("COLL_REPORT_ALL_BAD_COLL", new List<string> { "Всего найдено вобов: ", "Found vobs count: ", "Anzahl gefundener Vobs: ", "Ilość znalezionych vobów: ", "" });
            words.Add("COLL_REPORT_ALL_NOT_CATALOG", new List<string> { "Всего вобов вне Каталога: ", "Vobs which are not in Catalog: ", "Vobs außerhalb des Katalogs insgesamt: ", "Voby które nie znajdują się w Katalogu: ", "" });
            words.Add("REPORT_ALL_CONSOLE", new List<string> { "Информация выведена в консоль!", "All info is in console!", "Informationen wurden in der Konsole ausgegeben!", "Informacje znajdują się w konsoli!", "" });


            words.Add("ERROR_REPORT_AUTO_FIX_BUTTON", new List<string> { "Автоматически исправить", "Auto fix some problems", "Probleme automatisch beheben", "Napraw automatycznie", "" });
            ...
            words.Add("MSG_COMMON_FILE_PATH_SAVE_VOB_MESH", new List<string> { "Сохранять MESH вобов можно только внутри папки _WORK/DATA/MESHES/", "You can save vob's mesh only inside _WORK/DATA/MESHES/ folder!", "Du kannst VOB Meshes nur im Ordner _WORK/DATA/MESHES/ speichern!", "", "" });
            ...
            words.Add("COMMON_WARN_NOT_USUAL_VISUAL", new List<string> { "Тип воба не zCVob, а визуал 3DS или TGA. Вы уверены?", "The type of the vob is not zCVob, but visual has 3DS or TGA. Are you sure?", "Der Typ des Vobs ist nicht zCVob, aber das Visual hat 3DS oder TGA. Bist du sicher?", "", "" });
